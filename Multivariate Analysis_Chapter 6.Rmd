...
Calculate Bartlett's Sphericity to check if the variables are independent
...
From the output, it can be seen that the number of factors is equal to 5.
...
from the output, known that the for PC5 only has 1 variable. The goals for factor analysis is "grouping" thus we don't want that PC has only 1 variable. So we reduce the number of factors
...
Rotate the pc because from the previous PC, product reviews is not about fincance like others features in pc1
...
it shows that the results is the same when we use the fa fuction with fm = pa and rotate varimax
