...
PCA is concerned with explaining the variance-covariance structure of a set of variables through a few linear combinations of these variables. The objectives of PCA are data reduction and interpretation.
...
-   23 rows because from 27 athlete, there are only 23 active athlete (the first 23 data).

-   10 column, because we only apply PCA to the independent variables, which are the first 10 column
...
To know weather each variable has linear correlation or not is by drawing a diagonal line from each plot. If we can draw a diagonal line and the dot is following the line, then the variables have linear correlation. From the output, it shows that Long.jump and X100m have negative linear correlation.
...
If we want to reduce to 80% variable, we select only 4 pc (look at the cumukative). Because the cumulative proportion in PC4 is 0.80213 = 80%.
...
If we use scale = TRUE, the average of eigen value will always 1 and the total variance is equal to the number of variable

```{r}
res_eig
```

Since the eigen value of PC 1,2,3 \> mean of eigen value =1, then we use PC1, PC2, and PC3.
...
To perform the ***coordinates*** of the variables or the ***correlation*** between the PCA and original data

```{r}
var$coord
```

If the value is negative, it means that the correlation between original variable and the PC is opposite.
...
The color indicates the value of correlation, orange means the variable is perfectly represented by the PCs because the cos2 is high, blue means the variable is not perfectly represented by the PCs because the cos2 is low.
