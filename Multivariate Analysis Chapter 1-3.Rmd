...
In general, consider the q linear combinations of the p random variables 1,2,...,p.
The computation:
```
# Input Part
mu = matrix(c(),,)
...
df_ij
```
Explaination :Merge 2 dataframe by variable inside " ", then the output will show only for the same value of variable inside " "
...
```
Explaination:
Merge 2 dataframe by variable inside " ", then the output will show all of the data and the missing data will show NA.
...
df_L
```
Explaination:Combine right table (df2) to the left table (df1) by variable inside " " from the left table (df1)
...
df_R
```
Explaination:Combine left table (df1) to the right table (df2) by variable inside " " from the right table (df2).
...
results2
```
Explainatuon : is.na to check missing value, 2 means we check the column. if we use 1, means we check the row
...
data
```
B. Normality Test for multivariate variables using 'energy' Test
H0: data is multivariate normal distribution
vs H1: data is not multivariate normal distribution
For conclusion, we will reject H0 if p-value < 0.05
...
```
1.2 Hotelling's Testing using packages
Installing the package:
```{r}
install.packages("MASS")
...
1.2.2 F Approximation
The Computation (continuity from X2 Approximation):
```{r}
# Computation Part
...
2.2 Hypothesis Testing Using Package
Installing the package
```{r}
...
Simulate 2 samples from two independent samples with common covariance matrix
```{r}
# Input Part
## Sample 1
vecmu1 = matrix(c(),,)
...
- var.equal = assumption of the variance, TRUE if equal and FALSE if not equal
...
3.2 Hypothesis Testing Using Package
Installing the package
```{r}
install.packages("corpcor")
install.packages("Hotelling")
...
Htest2
```
Note : The calculation for simultaneous confidence intervals is the same with two sample independent sample (sigma1=sigma2)
